...
def train(
    ...
    lr: float = 1e-3,
    device: str = "cpu",
    single_prob: float = 0.3,  # 使用“单一编队”样本的概率
) -> str:
    ...
    for step in range(1, steps + 1):
        ...
        # ====== 修改点 2：可把迭代数略增（可选） ======
        P = model(X, Y, sinkhorn_iters=50, tau=0.5)  # 甚至 0.25 更尖
        Np1, Mp1 = P.shape
        N, M = Np1 - 1, Mp1 - 1
        ...
        targets = []
        rows = []
        for i in range(N):  # 每一行都监督
            tgt_j = make_row_targets(N, M, mapping, i)  # 若无匹配则返回 M（dustbin 列）
            targets.append(tgt_j)
            rows.append(P[i, :])

        rows = torch.stack(rows, dim=0)  # [N, M+1]
        targets = torch.tensor(targets, dtype=torch.long, device=device)  # [N]
        loss = F.nll_loss(torch.log(rows + 1e-12), targets)
