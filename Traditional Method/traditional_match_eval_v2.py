...
def visualize_triplet(out_png: str,
    ...
    if 0 <= T1 < N1:
        ax1.scatter(P1[T1, 0], P1[T1, 1], s=160, facecolors="none", edgecolors="r", linewidths=2.0,
                    label=f"查询 T={T1 + 1}")
        ax1.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15))  # ✅ 不遮挡，推荐位置
        ax1.set_title("左：目指（圈出查询）")
    ...
    if 0 <= pred_j < N2:
        ax2.scatter(P2[pred_j, 0], P2[pred_j, 1], s=160, facecolors="none", edgecolors="r", linewidths=2.0,
                    label=f"Pred={pred_j + 1}")
        ax2.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15))  # ✅ 不遮挡，推荐位置
        ax2.set_title("中：测量（蓝=GT 红=Pred）")
...
# —— 导入新模型 —— #
from model_traditional_v2 import predict_query_match  # 就用新模型的函数

def main(data_dir: str = "Data/data",
         out_dir: str = "Traditional Method/visualization_traditional_v2",
         max_files: int = 1000,
         annotate_index: bool = False):
    ...
    for i, fname in enumerate(files, start=1):
        ...
        try:
            ...
            pred_j, probs, _dbg = predict_query_match(
                P1, T1, P2,
                k_sig=7,                     # 距离签名邻居数
                nbins_r=5, nbins_theta=12,   # SC 分箱
                w_sc=1.0, w_sig=0.30,        # 融合权重（SC 为主，签名做微调）
                rotation_invariant=True      # 角度循环移位，旋转不变
            )
